...
const Nganhsp = ({ navigation }) => {
    ...
    const handleDone = () => {
        if (selectedCategory) {
            // Người dùng đã chọn một danh mục, thực hiện xử lý tại đây
            Alert.alert('Đã chọn:', selectedCategory);
        } else {
            // Hiển thị cảnh báo nếu không có danh mục nào được chọn
            Alert.alert('Cảnh báo', 'Vui lòng chọn một danh mục trước khi nhấn "Xong"');
        }
    }
    return (
        <View style={styles.container}>
            ...
            <TouchableOpacity
                style={[styles.form, selectedCategory === 'Quần' && styles.selectedText]}
                onPress={() => click('Quần')}>
            ...
            <TouchableOpacity
                style={[styles.form, selectedCategory === 'Trang sức' && styles.selectedText]}
